...
def load_data(normal_stat=False):
    if normal_stat:
        ...
        with open(filepath, "rb") as data_file:
            x = cPickle.load(data_file, encoding='latin1')
...
def extract_fea(data, num_stat=9):
...
def gen_fea(data, time_steps=20, num_stat=9):
    """
	input: 
	...
	"""
    data_num = data[0].shape[0]
    len_seq = data[0].shape[1]
    ...
    if window_len == len_seq:
        ...
        new_data = np.ones((data_num, time_steps, num_stat), dtype=np.object)
        for idxdata, sig_data in enumerate(data):
